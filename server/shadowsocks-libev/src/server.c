...
static int
count_connections_for_ip(const char *ip)
{
    ...
    cork_dllist_foreach_void(&connections, curr, next) {
        ...
        if (server->peer_ip[0] != '\0' && strcmp(server->peer_ip, ip) == 0) {
            count++;
        }
    }
...
static int
ss_should_iplock_port(const char *port_str)
{
    const char *min_env = getenv("SS_IPLOCK_MIN");
    const char *max_env = getenv("SS_IPLOCK_MAX");
    int port, min_port, max_port;

    if (min_env == NULL || max_env == NULL) {
        return 1; /* varsayilan: tum portlari kilitle */
    }

    port     = atoi(port_str);
    min_port = atoi(min_env);
    max_port = atoi(max_env);

    if (port <= 0 || min_port <= 0 || max_port <= 0) {
        return 1;
    }
...
static int
check_ip_lock_allowed(const char *peer_ip)
{
    const char *locked;
...
accept_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (peer_name != NULL) {
        strncpy(server->peer_ip, peer_name, INET6_ADDRSTRLEN - 1);
        server->peer_ip[INET6_ADDRSTRLEN - 1] = '\0';
    }
    update_ip_lock_status();
    ev_io_start(EV_A_ & server->recv_ctx->io);
    if (!ip_lock_is_enabled()) {
...
main(int argc, char **argv)
{
...
#ifndef __MINGW32__
    if (remote_port != NULL && ss_should_iplock_port(remote_port)) {
        char lock_path[512];
    ...
    } else if (remote_port != NULL) {
        LOGI("ip-lock disabled for port %s (multi-device)", remote_port);
    }
#endif
