...
write_port_lock_ip(const char *port, const char *ip)
{
    ...
    if (port == NULL || port[0] == '\0' || ip == NULL || ip[0] == '\0') {
        return -1;
    }
...
static int
clear_port_lock_ip(const char *port)
{
    char path[512];
...
static int
parse_port_ip(char *buf, int len, char *port, char *ip)
{
    char *data = get_data(buf, len);
    char error_buf[512];
    json_settings settings = { 0 };
    ...
    if (obj->type == json_object) {
        ...
        for (i = 0; i < obj->u.object.length; i++) {
            ...
            if (strcmp(name, "server_port") == 0) {
                if (value->type == json_string) {
                    strncpy(port, value->u.string.ptr, 7);
                } else if (value->type == json_integer) {
                    snprintf(port, 8, "%" PRIu64 "", value->u.integer);
                }
            } else if (strcmp(name, "ip") == 0 && value->type == json_string) {
                strncpy(ip, value->u.string.ptr, INET6_ADDRSTRLEN - 1);
            }
        }
    }
...
static int
parse_port_only(char *buf, int len, char *port)
{
    char ip[INET6_ADDRSTRLEN];
    return parse_port_ip(buf, len, port, ip);
}

static int
read_port_status(const char *port, char *out, size_t out_size)
{
    char lock_path[512];
    ...
    if (f != NULL) {
        size_t n = fread(status_body, 1, sizeof(status_body) - 1, f);
        status_body[n] = '\0';
        fclose(f);
        if (status_body[0] != '\0') {
            snprintf(out, out_size, "%s", status_body);
            return 0;
        }
    }
...
manager_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (strcmp(action, "add") == 0) {
    ...
    } else if (strcmp(action, "set_ip") == 0) {
        char port[8];
        char ip[INET6_ADDRSTRLEN];
        char msg[128];
        int msg_len;

        if (parse_port_ip(buf, r, port, ip) == -1 || ip[0] == '\0') {
            LOGE("invalid set_ip command: %s", buf);
            goto ERROR_MSG;
        }

        if (write_port_lock_ip(port, ip) == -1) {
            goto ERROR_MSG;
        }
        ...
        if (ret != NULL) {
            struct server *server = (struct server *)ret;
            strncpy(server->locked_ip, ip, INET6_ADDRSTRLEN - 1);
            server->locked_ip[INET6_ADDRSTRLEN - 1] = '\0';
        }

        msg_len = snprintf(msg, sizeof(msg), "ok");
        if (sendto(manager->fd, msg, msg_len, 0, (struct sockaddr *)&claddr, len) != msg_len) {
            ERROR("set_ip_sendto");
        }
    } else if (strcmp(action, "clear_ip") == 0) {
        char port[8];

        if (parse_port_only(buf, r, port) == -1) {
            LOGE("invalid clear_ip command: %s", buf);
            goto ERROR_MSG;
        }

        clear_port_lock_ip(port);

        void *ret = cork_hash_table_get(server_table, (void *)port);
        if (ret != NULL) {
            struct server *server = (struct server *)ret;
            server->locked_ip[0] = '\0';
        }

        char msg[3] = "ok";
        if (sendto(manager->fd, msg, 2, 0, (struct sockaddr *)&claddr, len) != 2) {
            ERROR("clear_ip_sendto");
        }
        ...
        read_port_status(port, status, sizeof(status));
        if (sendto(manager->fd, status, strlen(status), 0, (struct sockaddr *)&claddr, len)
            != (ssize_t)strlen(status)) {
            ERROR("ip_status_sendto");
        }
    }
